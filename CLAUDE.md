...
- The English holiday countdown pages (`christmas.html`, `ramadan.html`, … one per slug in `data/holidays-en.js`) and `countdowns.html`, their Vietnamese twins `vi/<slug>.html` + `vi/countdowns.html` and the Arabic `ar/<slug>.html`, plus the `<!-- Holiday countdown pages (EN) -->` sitemap block (all languages) — rendered from `data/holidays-en.js` through `templates/holiday-countdown.html` / `templates/holidays-index.html`. Edit data/templates, then `npm run generate-holidays`. Their share images `assets/images/og/<slug>.jpg` (+ `countdowns.jpg`, `vi-*.jpg`, `ar-*.jpg`) come from `npm run render-holiday-og` (`-- en|vi|ar` renders one language only) (needs local Chrome + macOS `sips`, emoji via Noto Color Emoji); they contain no year, so re-render only when adding a holiday or changing its name/colors. `table`-rule holidays (Diwali, Hanukkah, CNY, Ramadan, Eid) need their date lists extended before they run out; `test/holiday-pages.test.js` fails when fewer than two future dates remain.
