...
- Affiliate links must use `target="_blank"` and `rel="noopener noreferrer sponsored nofollow"` in article/detail contexts.
...
When adding products, update `../tet-count-down-api-v2/affiliate/tiki/products` in the same operation. The mobile apps still decode `category` as a single legacy string, so use the crawler's sync option instead of copying website records directly.
